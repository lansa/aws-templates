...
$output = (Get-CFNStack -StackName $(TestStack) -region ap-southeast-2).Outputs
$websiteUrl = $output | Where-Object {$_.OutputKey -eq "WebsiteURL"}
$url = $websiteUrl.OutputValue
# Autoscaling Instance Id

$childstack = (Get-CFNStack | Where-Object {$_.StackName -match "$(TestStack)-Master" }).StackName[0]
