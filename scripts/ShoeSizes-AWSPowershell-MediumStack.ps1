...
$output = (Get-CFNStack -StackName $(stacknamemedium) -region ap-southeast-2).Outputs
$websiteUrl = $output | Where-Object {$_.OutputKey -eq "WebsiteURL"}
$url = $websiteUrl.OutputValue

# Autoscaling Instance Id

$childstack = (Get-CFNStack | Where-Object {$_.StackName -match "$(stacknamemedium)-Master" }).StackName[0]
